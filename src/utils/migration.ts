...
/**
 * Data Migration Utilities for DDS Validation Project
...
export async function fixMissingPostcodes(): Promise<MigrationResult> {
    ...
    try {
        ...
        const countQuery = `
            ...
            WHERE postcode IS NULL OR postcode = '' OR postcode = 'null'
        `;
        ...
        const updateQuery = `
            ...
            SET postcode = '1234', updated_at = NOW()
            WHERE postcode IS NULL OR postcode = '' OR postcode = 'null'
        `;
...
export async function identifyNonCompliantRecords(): Promise<NonCompliantRecord[]> {
    ...
    try {
        ...
        const query = `
            SELECT id, po_number, delivery_id, postcode
            FROM reference_submissions 
    ...
    try {
        ...
        const statsQuery = `
            SELECT 
                ...
                COUNT(CASE WHEN postcode IS NULL OR postcode = '' OR postcode = 'null' THEN 1 END) as missing_postcode,
